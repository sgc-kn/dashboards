...
function createStationCard(title, stations, meta, heatmapData) {
    ...
    // ✅ Satellitenkarte (Esri World Imagery)
    L.tileLayer(
...
function updateStationUI(side, stationName, barChart, chartIndex, heatmapData) {
    ...
    // Karte updaten
    const { lat, lon } = meta;
    side.map.setView([lat, lon], MAP_ZOOM); // Zoom 18 für 50 m gut sichtbar
    ...
    // 50 m Kreis setzen
    // Kreis (Meter-basiert). Wenn vorhanden: updaten, sonst anlegen.
    if (side.circle) {
        side.circle.setLatLng([lat, lon]).setRadius(RADIUS_METERS);
    } else {
    ...
    barChart.values
        ...
        .attr("x", (_, i) => {
            ...
            const offset = 20;
            return 100 + Math.max(bar1End, bar2End) + offset;
        });
}
